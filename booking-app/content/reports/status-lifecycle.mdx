...
tags: [lifecycle, status, state-machine, handoff, observability, coupons]
date: 2026-05-11
updated: 2026-06-05
order: 20
pills:
...
flowchart LR
    ...
    B -->|gateway / self-collect / monthly / coupon-comp| C[Payment Complete]
    B -->|operator cancels| D[Discarded]
    B -->|idle &gt; 30 min| E[Abandoned]
    E -->|rescued by reconcile| C
    E -->|operator resumes + applies coupon| B
...
| <Pill variant="ok">Payment Complete</Pill> | Payment confirmed (PayFast ITN, self-collect mark, monthly auto-mark, R0 coupon-comp) OR rescue from Abandoned | In Progress, Abandoned, Payment Complete (failed handoff retry) |
| <Pill variant="ok">Successful</Pill> | CareFirst auto-register returned 2xx | Payment Complete |
| <Pill variant="mute">Abandoned</Pill> | No activity for &gt; 30 minutes mid-flow | In Progress. **Can be resumed back to In Progress** when an operator applies a coupon on the payment page (since 2026-06-05) — `POST /api/coupons/apply` accepts Abandoned bookings and flips the status in the same write. |
...
<b>Yellow</b> — PayFast gateway · <b>Amber</b> — Self-collect at unit · <b>Blue</b> — Monthly invoice · <b>Emerald</b> — Coupon comp (R0 booking)
<br/><br/>
This is purely informational; the canonical status is still <b>Payment Complete</b> regardless of route.
</Callout>

<Callout title="Abandon/Discard releases the coupon" variant="warn">
When a booking with an attached coupon flips to <b>Abandoned</b> or <b>Discarded</b>, a Postgres BEFORE-UPDATE trigger deletes the corresponding <code>coupon_uses</code> row, clears the denormalised coupon columns on the booking, and resets <code>payment_amount</code> back to <code>original_amount</code>. The slot returns to the coupon's usage pool. See the Coupons section of the <a href="/reports/booking-flow">booking flow walkthrough</a>.
</Callout>
