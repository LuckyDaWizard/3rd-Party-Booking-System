...
Today our SSO handoff sends a **single hard-coded `clientCode`** for every booking, regardless of which clinic captured it. As soon as a second clinic onboards, their patients silently land in the first clinic's CareFirst tenant. We need to switch to **per-clinic `clientCode`** resolved from the booking's owning client row.

This RFC documents the proposed change and the **four questions** we need CareFirst's dev team to answer before we start implementation.

<Callout variant="warn" title="Implementation gated">
Until CareFirst confirms that the <code>auto-register</code> endpoint accepts arbitrary <code>clientCode</code> values per request (rather than deriving it from <code>x-api-key</code>), we can't safely build this. The answers also determine how we model the <code>uniqueReference</code> field and whether we need separate API keys per client.
</Callout>
...
1. **CareFirst confirms** the four questions below (gating step — see Callout at top of doc).
2. **CareFirst issues** the per-client `clientCode` + `planCode` pairs for each currently-active client. They land in a secure channel (1Password vault / encrypted email).
...
We need answers on these before we start. They each materially affect the implementation shape.

<Callout title="Q1 — Does auto-register accept arbitrary clientCode per request?">
Today we send <em>one</em> hard-coded <code>clientCode</code>. We want to send a different <code>clientCode</code> per booking (per the owning client of the unit the booking was captured at).

<strong>Is the endpoint accepting <code>clientCode</code> from the request body authoritative?</strong> Or is it cross-checked against the <code>x-api-key</code> we authenticate with? If the latter, we'll need a separate API key per client (Q4) — which materially changes our deployment shape.
</Callout>

<Callout title="Q2 — uniqueReference scope">
<code>uniqueReference</code> is currently our booking UUID. UUIDs are unique across our entire database (and therefore across all clientCodes we'd send), but we want to confirm:

<strong>Does CareFirst dedupe <code>uniqueReference</code> globally, or per <code>clientCode</code>?</strong> If globally — fine, our UUIDs are already globally unique. If per-clientCode — also fine, but worth confirming so we can document the contract.
</Callout>

<Callout title="Q3 — Per-client codes — issuance + format">
For each of our currently-active clients we'll need a <code>clientCode</code> and a <code>planCode</code>.

<strong>What's the process for issuing them — does our partnerships contact send them over, or is there a self-service portal?</strong> Any format / case-sensitivity / character-set constraints we should validate against in the Settings UI?
</Callout>

<Callout title="Q4 — Separate API keys per client?">
If the answer to Q1 is "<code>clientCode</code> is cross-checked against <code>x-api-key</code>", then we'll need one <code>x-api-key</code> per client.

<strong>If so:</strong> can we store them on the client row alongside the codes (encrypted at the DB layer), or do we keep them in env vars keyed by client ID? <strong>Either is acceptable</strong> — we just need to know which shape your team is recommending.
</Callout>
