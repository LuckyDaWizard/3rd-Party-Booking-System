...
Our SSO handoff currently uses the **single `clientCode` + `planCode` pair** we have on file — held in an environment variable on our side — so every booking lands in the same CareFirst account. As we prepare to bring Local Choice live, and International SOS and Zoie after them, we'd like to resolve `clientCode` + `planCode` **per booking** from the owning client record on our side, so each clinic group's patients are associated with the correct CareFirst account.

The work on our side is small (~half a day of engineering). What we'd need from your team is **one `clientCode` + `planCode` pair per clinic group** (one each for Local Choice, International SOS, and Zoie), plus a quick confirmation on the auth model (one shared `x-api-key` vs. one per client).
...
3. **System admin populates** the codes on the Settings tab for one client first (e.g. Local Choice as the first to go live).
...
The pair we currently have on file (the one in the Postman collection) was for development / testing and isn't assigned to any of our production clinic groups. To associate each clinic group's patients with the correct CareFirst account, we'd need a distinct <code>clientCode</code> + <code>planCode</code> pair issued for each:

<ul>
<li><strong>Local Choice</strong> — new pair</li>
