...
- Payment collection (PayFast gateway, self-collect at unit, monthly invoice, plus per-client discount coupons including a R0-comp path)
...
| Hosting | Docker on Hostinger VPS | Traefik reverse proxy; HTTPS live at [bookings.carefirst.co.za](https://bookings.carefirst.co.za) with auto-renewed Let's Encrypt cert |
...
There is currently **no inbound** channel from CareFirst to us. That's intentional in v1, but it limits what we can observe — see [Status lifecycle](/reports/status-lifecycle) for the consequences.
...
1. **Consultation-outcome webhook** — so we can mark bookings <Pill variant="ok">Consultation Complete</Pill> (vs the current `Successful = handed off`) and improve month-end invoice accuracy
2. **Stable `externalReferenceId` in the auto-register response** — so we can quote it in support tickets without doing opportunistic parsing
