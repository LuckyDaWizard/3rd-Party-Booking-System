...
internal: true
tags: [patient-history, operator, reconcile, payfast, carefirst, daily, coupons]
date: 2026-05-11
updated: 2026-06-08
pills:
...
**All · In Progress · Incomplete · Payment Complete · Successful · Abandoned · Discarded**. "Incomplete" is the aggregated catch-all for stuck states. **Discarded** is admin-only (system_admin / unit_manager) and surfaces bookings the operator explicitly discarded.
</Card>
...
- **Emerald** — coupon comp (R0 booking — coupon covered the full fee)
...
<Callout title="Coupon chip">
Bookings that had a coupon applied show a small emerald chip under the patient name:

- <code>CODE · −R25.00</code> — discounted booking (still went through PayFast at the reduced amount)
- <code>CODE · Comped</code> — R0 booking, completed without PayFast via the comp endpoint

The chip is absent for bookings that never had a coupon, or where the coupon was released because the booking was Abandoned/Discarded.
</Callout>
...
- Payment Type, Coupon Code, Original Amount, Discount Amount, Final Amount
- Vitals (BP, glucose, temperature, oxygen saturation, heart rate) when captured
...
<Card variant="warn" title="Coupon was applied but the patient bailed">
No action needed — the system auto-releases the coupon slot when the booking flips to Abandoned/Discarded. The coupon's usage count goes back down by one and the same code can be re-applied to the patient's next attempt (limits permitting).
</Card>
</Grid2>
