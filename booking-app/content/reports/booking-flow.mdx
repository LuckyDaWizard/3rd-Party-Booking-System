...
tags: [patient-journey, capture, payfast, carefirst, flow, coupons]
date: 2026-05-11
updated: 2026-06-08
order: 10
pills:
...
flowchart LR
    ...
    E --> CP{Coupon to R0?}
    CP -->|no| H[/payment/success/]
    CP -->|yes| CC[Comp endpoint]
    CC --> H
    F --> I[/patient-metrics/]
    G --> I
    H --> I
    ...
    class B,C,E,H,I,J,K,CP brand;
    class CC ok;
    class L ok;
```

Each rounded box = a page. Diamond = branch resolved from the parent client's settings (or, for the coupon question, from the booking's payment_amount after the coupon is applied).
...
| `allow_coupons` | Expose the Apply coupon box on the payment step | Step 5 (gateway mode only) |
...
**Mutual exclusion:** `collect_payment_at_unit` and `bill_monthly` cannot both be ON. UI cascades the toggles; server PATCH clamps if both ever arrive TRUE. `allow_coupons` is independently mutually exclusive with both non-gateway modes — coupons only make sense on the PayFast (gateway) path; the API guards reject coupon applies on self-collect / monthly-invoice bookings.
...
flowchart TD
    ...
    C2 --> F
    F --> CP{Apply coupon?}
    CP -->|no| G[/payment/success/]
    CP -->|yes, total &gt; 0| F
    CP -->|yes, total = R0| CC[POST complete-coupon-comp]
    CC --> G
...
<Section id="coupons" num="13 — Coupons" title="Discount codes on the gateway path">

Per-client opt-in feature. Operators (or patients on a payment link) can apply a coupon code on the payment step to reduce the consultation fee. Gated by `clients.allow_coupons = TRUE` and only available on the gateway flow — coupons are mutually exclusive with self-collect and monthly-invoice.

```mermaid
flowchart LR
    A[Payment step mounted] --> B{allow_coupons?}
    B -->|FALSE| C[No coupon box]
    B -->|TRUE| D[Apply coupon input shown]
    D --> E[POST /api/coupons/apply]
    E -->|invalid / expired / over-limit| F[Inline error, no DB change]
    E -->|valid| G[Insert coupon_uses row<br/>+ denorm coupon_id/code/discount/amount on booking]
    G --> H{Final amount &gt; 0?}
    H -->|yes| I[Pay With PayFast<br/>at discounted amount]
    H -->|no, total = R0| J[Complete Free Booking<br/>POST complete-coupon-comp]
    J --> K[Status = Payment Complete<br/>payment_type = coupon_comp]
    I -->|ITN ok| L[Status = Payment Complete<br/>payment_type = gateway]

    classDef brand fill:#e0f0fa,stroke:#1f6f9d,color:#0f172a;
    classDef ok    fill:#dcfce7,stroke:#166534,color:#0f172a;
    classDef err   fill:#fee2e2,stroke:#991b1b,color:#0f172a;
    classDef warn  fill:#fef3c7,stroke:#92400e,color:#0f172a;
    class A,D,E,G,I brand;
    class K,L ok;
    class F err;
    class J warn;
```
...
- **Percentage** — % off the consultation fee. Bounded `(0, 100]`. 100% = full comp.
- **Fixed** — flat rand amount off. If the value exceeds the consultation fee, the final amount is clamped to R0 (still triggers the R0 comp path).

### Limits enforced server-side

| Limit | Source | Enforcement |
|---|---|---|
| `valid_from` / `valid_to` window | `coupons` row | Apply endpoint rejects with "Code not valid yet" / "Code expired" |
| `usage_limit` (total) | `coupons` row | Apply endpoint counts existing `coupon_uses` rows |
| `usage_limit_per_email` | `coupons` row | Apply endpoint counts `coupon_uses` matching `lower(patient_email)` |
| `min_spend` | `coupons` row | Apply endpoint rejects if booking amount &lt; min_spend |
| Per-client `client_id` scope | `coupons.client_id` | When non-NULL, apply rejects if booking unit's parent client differs |
| Mutex with non-gateway modes | API guard | Apply rejects if `payment_type IS NOT NULL` and not gateway |

### Lifecycle — what happens on abandon / discard

```mermaid
flowchart LR
    A[Booking with coupon attached] --> B{status flips}
    B -->|Payment Complete / Successful| C[Coupon usage retained<br/>counts against usage_limit]
    B -->|Abandoned / Discarded| D[Postgres BEFORE-UPDATE trigger fires]
    D --> E[DELETE coupon_uses row]
    D --> F[NULL coupon_id, coupon_code, discount_amount]
    D --> G[Reset payment_amount = original_amount]
...
The trigger is `SECURITY DEFINER` so it works regardless of caller (browser JWT or server service-role). Without this, browser-side discards fail silently because `coupon_uses` is RLS-protected.

**Result:** A 100-use coupon doesn't get silently exhausted by abandoned carts. The slot returns to the pool the moment the booking falls out of the active funnel.

### Resuming an Abandoned booking via coupon apply (2026-06-05)

The mirror case: an operator returns to a booking the idle-timer has flipped to `Abandoned` and applies a coupon to bring the patient back into the funnel. `POST /api/coupons/apply` accepts `status = "Abandoned"` (alongside `In Progress`) and, in the same atomic write:

1. Records the coupon usage on the `coupon_uses` row
2. Denormalises `coupon_id` / `coupon_code` / `discount_amount` / `payment_amount` on the booking
3. **Flips `status` back to `In Progress`** — the operator is actively working on it again

Pre-hotfix this returned 409 with "Coupon can only be applied while the booking is in progress", forcing the operator to manually un-abandon first. The regression net at `booking-app/tests/coupon-r0-happy-path.spec.ts` covers this path.

### R0 comp endpoint — why it exists

PayFast (like most gateways) rejects transactions with amount = 0. A 100%-off code, or a fixed-amount code worth more than the consultation fee, lands the booking on R0. `POST /api/bookings/[id]/complete-coupon-comp` is the dedicated path for that case:

- Strict preconditions: status = In Progress, `coupon_id IS NOT NULL`, `payment_amount = 0`.
- Bypasses PayFast entirely.
- Sets `payment_type = "coupon_comp"` (distinct from `gateway` / `self_collect` / `monthly_invoice`) so reports can count free bookings separately.
- Records the operator on `booking_validator_id` / `booking_validator_name` like the other non-gateway completion paths.

</Section>

<Section id="post-payment" num="14 — Post-payment" title="Vitals, terms, handoff">
...
<Section id="handoff" num="15 — CareFirst handoff" title="SSO auto-register">
...
<Section id="lifecycle" num="16 — Lifecycle" title="Booking statuses">
...
| <Pill variant="ok">Payment Complete</Pill> | Paid, awaiting Start Consult | Yellow pill (gateway), amber (self-collect), blue (monthly), emerald (coupon comp) |
...
<Section id="cross-cutting" num="17 — Cross-cutting" title="Cross-cutting concerns">
...
<Section id="code-map" num="18 — Code map" title="Where to look in code">
...
| Coupon apply / remove | `src/app/api/coupons/apply/route.ts`, `src/app/api/coupons/remove/route.ts` |
| Coupon R0 comp | `src/app/api/bookings/[id]/complete-coupon-comp/route.ts` |
| Coupon abandon-release trigger | `supabase/migrations/036_*.sql` + `037_*.sql` (SECURITY DEFINER) |
| Coupon admin pages | `src/app/(dashboard)/coupons/...` + `src/app/api/admin/coupons/...` |
