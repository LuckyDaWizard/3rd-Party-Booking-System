...
sections:
  ...
  - { id: step-4,          label: Step 4 — Booking type }
  - { id: step-5,          label: Step 5 — Verification }
  - { id: step-6,          label: Step 6 — Payment branches }
...
|---|---|---|
| `collect_payment_at_unit` | Skip PayFast — pay at unit, confirm with PIN | Step 6 |
| `bill_monthly` | Skip payment entirely; client invoiced at month-end | Auto-skip step 6 |
...
<Section id="patient-details" num="04 — Patient Details" title="6 steps in one page">

One page, six logical steps. Auto-saves every 2 seconds so a crash costs at most a sentence of typing.
...
<StepRow>
  <Step num="4." title="Booking Type">Instant or Scheduled (date + time)</Step>
  <Arrow />
  <Step num="5." title="Verification">Review + edit + nurse PIN if required</Step>
  <Arrow />
  <Step num="6." title="Payment Type">Branch on client settings</Step>
</StepRow>
...
- **Verify-step edits** (booking type / date / time) re-persisted on Next — auto-save's payload doesn't carry those
...
<Section id="step-4" num="07 — Step 4" title="Booking Type">

Defines whether the consultation runs immediately or is scheduled for later.

<Grid2>
<Card variant="ok" title="Instant Consult">
<Pill variant="ok">default</Pill>
Standard flow. After payment + handoff, the patient is seen straight away in the CareFirst Patient app.
</Card>

<Card variant="brand" title="Scheduled Consult">
Operator captures the patient now; CareFirst schedules the slot at `scheduled_at`.

- Date + Time pickers (modal-based for both)
- Stored UTC; displayed local
- Forwarded to CareFirst as `{ bookingType: "scheduled", scheduledAt }`
</Card>
</Grid2>

### Persistence

Fields persisted both at step 4 → 5 (initial pick) *and* step 5 → 6 (Verify-step edits). Step 4 forgets, step 5 catches.

### Errors handled

If "Scheduled" is picked without a date or time, Next stays disabled. We do **not** validate slot availability — CareFirst is the source of truth and surfaces conflicts post-handoff.

</Section>

<Section id="step-5" num="08 — Step 5" title="Verification">
...
<Section id="step-6" num="09 — Step 6" title="Payment branches">

Resolved server-side from the booking's parent client. UI auto-renders the right panel.

```mermaid
flowchart TD
    A[Step 6 mounted] --> B{client config}
...
For clients invoiced at month-end. The operator never sees Step 6.

```mermaid
flowchart LR
    A[Step 6 mounts] --> B["useEffect detects mode=&#39;monthly_invoice&#39;"]
