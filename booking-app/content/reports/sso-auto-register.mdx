...
|---|---|---|
| `clientCode` | Env var `CAREFIRST_CLIENT_CODE` | Per-environment, immutable per deploy. **Under active discussion to become per-client** — see [Multi-Client Routing RFC](/reports/multi-client-routing). |
| `planCode` | Env var `CAREFIRST_CLIENT_PLAN_CODE` | Same as `clientCode`. Pairs with it. Same multi-client RFC applies. |
...
1. **Per-client `clientCode` + `planCode`.** Today every booking sends the same hard-coded pair, so every patient lands in one CareFirst account. As we onboard additional clinic groups (Local Choice, International SOS, Zoie, future), each needs to land in its own CareFirst account. Full discussion + the two specific asks are in the [Multi-Client Routing RFC](/reports/multi-client-routing) — currently the single most material open item on this integration.
