...
tags: [sso, auto-register, contract, payload, api, handoff, vitals]
date: 2026-05-11
updated: 2026-06-03
order: 10
pills:
...
```json
{
  ...
  "user": {
    ...
    "additionalScriptEmail": "carer@example.com",
    "userProfile": {
...
<Callout title="Speculative extensions — not yet in your schema" variant="warn">
Two fields above are <strong>not</strong> defined in your current TypeScript interface or Postman collection: <code>user.additionalScriptEmail</code> and the entire <code>user.vitals</code> block. We send them speculatively starting 2026-06-03 — if your endpoint silently ignores unknown fields (most do), nothing changes for you. When you extend your schema to consume them, no code change is required on our side. If your endpoint <strong>rejects</strong> unknown fields under strict validation, let us know and we'll either gate behind a flag or remove until you're ready.

<br/><br/>

<code>additionalScriptEmail</code> in particular is the destination the patient asked us to send their prescription to — without it being forwarded, that intent is captured but never acted on. Worth confirming this surfaces somewhere usable on your side.
</Callout>
...
| `user.additionalScriptEmail` | `bookings.{script_to_another_email, additional_email}` | The patient's "script to another email" answer. Populated when the operator captured both the Yes toggle AND a non-empty email; otherwise `null`. Speculative — see callout below. |
...
| `user.vitals.bloodPressure` | `bookings.blood_pressure` | Composite string (e.g. `"120/80"`). Speculative — see callout. |
| `user.vitals.glucose` | `bookings.glucose` | mmol/L, string. Speculative. |
| `user.vitals.temperature` | `bookings.temperature` | Celsius, string. Speculative. |
| `user.vitals.oxygenSaturation` | `bookings.oxygen_saturation` | Percent, string. Speculative. |
| `user.vitals.heartRate` | `bookings.heart_rate` | bpm, string. Speculative. |
| `user.vitals.capturedAt` | `bookings.updated_at` | Best-effort "when these readings were last touched" (booking row's last-modified time). Speculative. |
| `returnUrl` | `<APP_URL>/patient-history` | We send the operator back here after the patient finishes |

The entire `user.vitals` block is **omitted from the payload** when every vital reading is null or empty — which is the case for `skip_patient_metrics` clients and for any booking that never reached the metrics step.
...
4. **Multiple environments.** Confirm staging/production endpoint URLs, API key issuance process, and any rate limits.
