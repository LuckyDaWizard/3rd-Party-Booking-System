...
- **Slot expired** — for scheduled bookings that the patient never used

All four of these resolve to <Pill variant="ok">Successful</Pill> on our side, because that status reflects only that the **handoff succeeded** — not that the consult happened.
...
| `no_show` | Patient never joined (scheduled bookings) | <Pill variant="mute">No-show</Pill> (new) |
| `expired` | Scheduled slot passed without consult | <Pill variant="mute">Slot expired</Pill> (new) |
...
| `expired` | Mark <Pill variant="mute">Slot expired</Pill>; **exclude** from monthly invoice |

Even just `completed` and `no_show` would close 80% of the gap. The other three are valuable but not blocking.
...
4. **Status enum** — are the five states above (`started/completed/cancelled/no_show/expired`) a reasonable approximation of what your system tracks? If you have additional states, list them.
