...
// context — same pattern as coupon-r0-happy-path.spec.ts).
...
async function readBooking(bookingId: string) {
  ...
  const { data } = await admin
    .from("bookings")
    .select(
      "status, payment_amount, original_amount, discount_amount, payment_type, coupon_id, coupon_code"
    )
    .eq("id", bookingId)
    .single()
  return data as {
...
//
// Identical to the R0 spec's helper — kept inlined rather than extracted into
// a shared module because Playwright tests don't share imports cleanly across
// files in this repo, and inlining matches the existing convention.
...
// ----- Fixture: a one-off coupon scoped to the seeded test client ------------
//
// Each test creates its own coupon so we don't mutate the seeded 100%-off
// fixture (the R0 spec depends on it staying intact). The code is suffixed
// with Date.now() so concurrent runs / parallel workers don't collide on the
// coupons.code_lower unique index (migration 038).
...
async function createDiscountCoupon(
  ...
  spec: {
    ...
    /** Code prefix — Date.now() suffix is appended automatically. */
    codePrefix: string
  }
): Promise<CreatedCoupon> {
  const admin = getAdmin()
  // Pad with random suffix in addition to Date.now() so two tests calling
  // this in the same millisecond can't trip the unique index. crypto.randomUUID
  // is overkill; a 4-char base36 slice is plenty.
  ...
  return {
    ...
    async cleanup() {
      ...
      // CASCADE) so the booking's coupon_uses row goes with this row even
      // if booking cleanup runs later.
...
async function getSeededUserId(): Promise<string> {
  const admin = getAdmin()
  const email = `pin-${SEED.user.pin}@carefirst.local`
  const { data } = await admin
    .from("users")
    .select("id")
    .eq("email", email)
...
async function signInAsSeededUser(page: import("@playwright/test").Page) {
  ...
  // robust approach is to focus the first slot and type the digits — same
  // pattern as coupon-r0-happy-path.spec.ts.
