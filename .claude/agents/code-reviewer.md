...
- [ ] **Double-click guard** on any new interactive submit handler that triggers an INSERT or financial mutation. `disabled={submitting}` alone is insufficient because React batches state updates — a second click within the same render frame slips through. Block: missing synchronous `useRef<boolean>` guard at the TOP of the onClick handler. Block: missing Promise-level dedup in the store action if the handler calls a store create/mutate function. Block: missing `try/finally` wrap (a thrown handler leaves the button stuck disabled). See memory `feedback_double_click_guard`; D19 production incident closed this gap on booking create.
